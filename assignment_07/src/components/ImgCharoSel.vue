...
<template>
  <div class="my-10">
    ...
    <button @click="imageSlider()" class="ml-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
      Button
    </button>

  </div>
  <div class="mx-auto items" id="carousel">
    <div :style="`background-image:url(${item})`" class="item" v-for="(item, index) in items" :key="index">{{ index + 1 }}
    </div>
  </div>
</template>
